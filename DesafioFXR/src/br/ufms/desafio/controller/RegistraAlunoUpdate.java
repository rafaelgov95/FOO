...
import br.ufms.desafio.model.dao.TelefoneDAO;
import java.net.URL;
 ...
 */
public class RegistraAlunoUpdate extends TrocarCena implements Initializable {
    ...
    @FXML
    private ComboBox<Telefone> cbTelefonesAdicionados;
    @FXML
...
//            
            ...
            TelefoneDAO TelefoneDAO = DAOFactory.getInstance().getTelefoneDAO();
            List<Telefone> listaT = TelefoneDAO.getAll();
            ObservableList<Telefone> telefoneColletion = FXCollections.observableList(listaT);
            for (int i = 0; i < listaT.size(); i++) {
                telefoneColletion.add(listaT.remove(i));
            }
//            
            cbTelefonesAdicionados.setItems(telefoneColletion);
//            cbUF.setValue(Buffer.user.getEndereco().getMunicipio().getUF().getNomeEstado());
//            cbMunicipio.setId(Buffer.user.getEndereco().getMunicipio());
        } catch (SQLException ex) {
            Logger.getLogger(RegistraAlunoUpdate.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//
    ...
    private void adicionarTelefone(ActionEvent event) {
        if (tfDdd.getText().equals("") || tfTelefone.getText().equals("")) {
            ...
            cbTelefonesAdicionados.setItems((ObservableList<Telefone>) listaAuxTel);
            tfDdd.setText("");
            tfTelefone.setText("");
    ...
    private void deletarTelefone(ActionEvent event) {
        Telefone telefoneDeletar = (cbTelefonesAdicionados.getValue());
        for (int i = 0; i < listaAuxTel.size(); i++) {
            System.out.println("----------------");
            if (listaAuxTel.get(i).equals(telefoneDeletar)) {
